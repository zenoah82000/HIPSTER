...
router.get("/map", async (req, res) => {
  ...
  const sqlproductlist =
    "SELECT `map_product`.`mapId`, `map_product`.`productId`,`product`.`companyId`, `map_product`.`star`, `map_product`.`created_at`, `map_product`.`updated_at`,`product`.`productId`,`product`.`productAddress`,`product`.`productName`,`product`.`productImg`,`product`.`categoryId`,`product`.`lat`,`product`.`log`,`product`.`category`,`product`.`productPrice`,`company`.`companyId`,`category`.`categoryName` FROM `map_product` LEFT JOIN `product` ON  `map_product`.`productId` =`product`.`productId`LEFT JOIN `company` ON  `product`.`companyId` =`company`.`companyId` LEFT JOIN `category` ON  `product`.`categoryId` =`category`.`categoryId`";
