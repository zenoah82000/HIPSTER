...
// 首頁>精選商品
  ...
  const Sql = "SELECT `productId`,`productImg`,`productName`,`productAddress`,`star`, `created_at`, `productEndingDate` FROM `product` ORDER BY `star` DESC LIMIT 3"
...
// 首頁>即將結束商品
  ...
  // console.log(typeof req.body)
  // console.log(req.body)
  const Sql = "SELECT `productId`,`productImg`,`productName`,`productPrice`,`productAddress`,`productEndingDate` `star`, `created_at` FROM `product` ORDER BY `productEndingDate` LIMIT 5 "
  const [r1] = await db.query(Sql);
  res.json(r1)
