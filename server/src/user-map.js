...
router.get("/mymap/:memberId", async (req, res) => {
  ...
  const sqlmyItemList =
    "SELECT `item_lists`.`itemListId`, `item_lists`.`orderId`, `item_lists`.`memberId`, `item_lists`.`date`, `product`.`productName`,`product`.`productAddress`,`product`.`lat`,`product`.`log`,`comments`.`commentImg`,`comments`.`content` FROM `item_lists` LEFT JOIN `product`ON `item_lists`.`productId` =`product`.`productId`  LEFT JOIN `comments` ON `item_lists`.`itemListId` = `comments`.`itemListId` WHERE `item_lists`.`memberId` = ?  AND `item_lists`.`date`<= CURDATE() and  `comments`.`commentImg` is not null ORDER BY `item_lists`.`date` DESC ";
